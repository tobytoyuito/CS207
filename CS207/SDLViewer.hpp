...
class SDLViewer {
 private:
  ...
  double dimensions_[6];   //[x_max, x_min, y_max, y_min, z_max, z_min]
  ...
  void add_nodes(InputIterator first, InputIterator last,
    ...
    { safe_lock mutex(this);
      ...
      for (; first != last; ++first) {
        ...
        if(pos.x>dimensions_[0]) dimensions_[0] = pos.x;
        if(pos.x<dimensions_[1]) dimensions_[1] = pos.x;
        if(pos.y>dimensions_[2]) dimensions_[2] = pos.y;
        if(pos.y<dimensions_[3]) dimensions_[3] = pos.y;
        if(pos.z>dimensions_[4]) dimensions_[4] = pos.z;
        if(pos.z<dimensions_[5]) dimensions_[5] = pos.z;
        center_ +=pos;
      }

      // Zoom out if graph is large
      if(auto_track_){
        if((dimensions_[0]-dimensions_[1])>camera_.distance()*0.75) camera_.zoom(1.2);
        else if((dimensions_[2]-dimensions_[3])>camera_.distance()*0.75) camera_.zoom(1.2);
        else if((dimensions_[4]-dimensions_[5])>camera_.distance()*0.75) camera_.zoom(1.2);
      }
 ...
 private:
  ...
  void init() {
    ...
    // Initialize dimensions of object
    dimensions_[0] = 0;
    dimensions_[1] = 0;
    dimensions_[2] = 0;
    dimensions_[3] = 0;
