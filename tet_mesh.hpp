/*
 * CS 207: Final Project Tetrahedral T_Mesh
...
#pragma once
/** @file tet_mesh.hpp
 * @brief A T_Mesh is composed of nodes, edges, and tetrahedrals such that:
...
class T_Mesh {
  ...
  public:
    /** Type of indexes and sizes. Return type of T_Mesh::num_nodes(). */
    ...
    class Node : private totally_ordered<Node> {
      public:
       ...
       /** Return this node's position as a constant.
        ...
        bool operator<(const Node& n) const {
          if( *this == n )
            return false;
          if( m_ == n.m_ ){// nodes in the same T_Mesh
            if( node_uid_ < n.node_uid_ ){
              return true;
            }
          }else{ // nodes not in the same T_Mesh
            if( m_ < n.m_ ){
              return true;
            }
          }
      ...
      private:
        /** Private constructor for T_Mesh to construct nodes
         * @param[in] mesh This new node's parent mesh.
         * @param[in] uid The uID that @a mesh uses to uniquely identify this node. @a uid >= 0.
         */
        Node( const T_Mesh* m, size_type node_uid )
            : m_( const_cast<T_Mesh*>(m) ), node_uid_(node_uid) {
        }
         ...
         */
        T_Mesh* m_; //Pointer to the parent mesh
        size_type node_uid_; //uid for the node in the parent mesh

        // Allow T_Mesh to access Node's private member data and functions.
        friend class T_Mesh;
    };
    ...
    class Edge : private totally_ordered<Edge> {
      ...
      private:
        ...
        /** Private constructor for T_Mesh to construct edges.
         ...
         */
        Edge(const T_Mesh* m, size_type node_uid1, size_type node_uid2 )
        		: m_(const_cast<T_Mesh*>(m)), node_uid1_(node_uid1), node_uid2_(node_uid2) {}
         ...
         */
        T_Mesh* m_; //Pointer to the parent mesh for the edge
...
//        			cross ( node(2).position() - node(0).position(), node(3).position() - node(0).position() )  )/6.0;
        ...
        bool operator<(const Tetrahedral& t) const {
          if( *this == t )
            return false;
          if( m_ == t.m_ ){// tetrahedral in the same T_Mesh
            if( tet_uid_ < t.tet_uid_ ){
              return true;
            }
          }else{ // tetrahedral not in the same T_Mesh
            if( m_ < t.m_ ){
              return true;
            }
          }
      ...
      private:
        /** Private constructor for T_Mesh to construct tetrahedrals
         ...
         */
        Tetrahedral( const T_Mesh* m, size_type tet_uid )
            : m_( const_cast<T_Mesh*>(m) ), tet_uid_(tet_uid) {
        }
    ...
    class NodeIterator : private equality_comparable<NodeIterator> {
     ...
     private:
      ...
      NodeIterator( const T_Mesh* m, typename g_real_type::NodeIterator nit )
          : m_( const_cast<T_Mesh*>(m) ), nit_(nit) {
      }
    };
    ...
    class EdgeIterator : private equality_comparable<EdgeIterator> {
     ...
     private:
      ...
      EdgeIterator( const T_Mesh* m, typename g_real_type::EdgeIterator eit )
          : m_( const_cast<T_Mesh*>(m) ), eit_(eit) {
      }
    };
    ...
    /** @class T_Mesh::TetrahedralIterator   TetrahedralIterator tet_iterator
    ...
    class TetrahedralIterator : private equality_comparable<TetrahedralIterator> {
     ...
     private:
      ...
      TetrahedralIterator( const T_Mesh* m, typename g_tet_type::NodeIterator tit )
          : m_( const_cast<T_Mesh*>(m) ), tit_(tit) {
      }
    };
    ...
    class IncidentIterator : private equality_comparable<IncidentIterator> {
     ...
     private:
      ...
      IncidentIterator( const T_Mesh* m, typename g_real_type::IncidentIterator icit )
          : m_( const_cast<T_Mesh*>(m) ), icit_(icit) {
      }
    };
     ...
     * @pre @a n0, @a n1, @a n2, @a n3 are valid T_Mesh::Node
